...
void state114()
{
  ...
  L1_V_MovD_hallD();
  L3_V_MovD_hallD();
  if (flag1vd == 1 && flag3vd==1 && digitalRead(LS_L1)==0 && digitalRead(LS_L3)==0 )
  {
    flag3_down=0;
    flag1_down=0;
    flag1vd = 0;
...
void state117()
{
  ...
  L2_V_MovD_hallD();
  L4_V_MovD_hallD();
  if (flag2vd == 1 && flag4vd==1 && digitalRead(LS_L2)==0 && digitalRead(LS_L4)==0) 
  {
    flag2vd = 0;
    flag4vd=0;
    L2V_sum = 0;
...
L2H_pwm_constrain = 190;
L3H_pwm_constrain = 90;
L4H_pwm_constrain = 80;
base1h=30;
base2h=30;
base3h=10;
base4h=10;
...
//    attachInterrupt(digitalPinToInterrupt(L3V_hall), ISR_3V, FALLING);
    ...
    if((digitalRead(mpu1)==0 && digitalRead(mpu2)==0) || (digitalRead(mpu1)==1 && digitalRead(mpu2)==1))
    {
      state =135;
    }
    else
...
//            state=0;
          ...
          {
           L1H_diff=map(dist,12,33,40,235);
...
//        state=0;
      ...
      {
       L4H_diff=map(dist,12,33,40,235);
...
L3H_pwm_constrain = 170;    
L4H_pwm_constrain = 170;
L2H_pwm_constrain = 100;
L1H_pwm_constrain = 100;
base3h=30;
base4h=30;
//  flag1_up=1;
//  flag3_up=1;
...
//    flag4vu=0;
    ...
    if(digitalRead(mpu1)==1 && digitalRead(mpu2)==1)
    {
//      state = 0;
